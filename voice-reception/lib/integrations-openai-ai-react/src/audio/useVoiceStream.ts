...
export function useVoiceStream({ workletPath, ...callbacks }: StreamCallbacks) {
  ...
  const streamVoiceResponse = useCallback(
    async (url: string, audioBlob: Blob): Promise<void> => {
      ...
      try {
        let audioPlaybackAvailable = true;
        try {
          await playbackRef.current.init();
        } catch (initErr) {
          // AudioWorklet unavailable (e.g. iOS gesture timing, missing HTTPS, unsupported browser).
          // Continue without audio playback — transcript will still be delivered.
          audioPlaybackAvailable = false;
          console.warn("[voice-stream] Audio playback init failed, continuing without it:", initErr);
        }
        throwIfNotCurrent();
  ...
  return { streamVoiceResponse, playbackState: playback.state, initAudio: playback.init };
}