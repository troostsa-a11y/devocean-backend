...
- **LCP is the static `#hero-title` text, NOT the hero image**: a full-viewport `<img>` (the `#hero-placeholder` background) is excluded from LCP by Chrome's "full-viewport = background" heuristic (web.dev/articles/lcp) — it only ever counts toward FCP. So the LCP element is whichever text block is largest: the static `#hero-title` (paints ~1.2 s, in raw HTML) vs. the CookieYes consent `<p>` (a long IAB-TCF "we and our N partners…" paragraph that paints ~9.7 s on Slow 4G). In the lab there is no user interaction, so a larger late banner is NOT discarded and supersedes early text — the title must therefore be sized to out-size the consent paragraph. `#hero-title` is `clamp(3.5rem, 14vw, 3.75rem)` (wraps to 2 lines on mobile), and the React `HeroSection` h1 mirrors that **exact** clamp so there's no shrink-jump at the fade. The CookieYes paragraph is also type-compacted (`.cky-notice-des` 13px / lh 1.4, copy preserved on-screen). If you shrink the title or the consent copy grows, the `<p>` can reclaim LCP. **No image change (resolution, aspect, asset swap) can ever fix hero LCP — the image is structurally ineligible.**
