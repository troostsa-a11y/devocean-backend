...
- **NR (non-refundable) rate plans are NOT offered to guests**: filtered out in `Beds24Service.priceOffers()` before they ever reach the availability response. Reason: NR is Beds24's cheapest offer when present, but requires 100% payment at booking — contradicting the "50% deposit, balance on arrival" promise shown throughout `/book-direct`. A SF offer always exists alongside NR so filtering it never affects room availability. The only non-refundable option guests see is LM (last-minute), which already correctly shows 100% deposit due. Do NOT re-expose NR rates without revisiting the deposit UX.
