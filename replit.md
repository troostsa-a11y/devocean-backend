...
- **Shared nav injector**: `public/js/shared-nav.js` injects the full two-tier header (language topbar + main nav) into every standalone HTML page (story, meals, accommodation, guide pages) via a single `<script src="/js/shared-nav.js">` at the top of `<body>`. Language changes navigate to the locale-prefixed URL (e.g. `/pt-pt/chalet`) via `setLang()` — no `?lang=` query param. Nav labels are translated inline for all 22 languages (`T` table). The topbar language control is a searchable picker (compact button + panel): 7 pinned common languages shown by default; typing filters all 22. No region selector — region is IP-detected silently.
- **Static-page content i18n pattern** (used by `story.html` and `devocean-lodge-meals.html`): tag translatable elements with `data-i18n="key.path"` (add `data-i18n-html` only when the string contains inline HTML like `<strong>`/`<span>`/`<br/>`); per-page loader script `public/translations/<page>-i18n.js` (language detection: `window.__DEVOCEAN_LOCALE__` injected by the edge → `?lang=` URL param → localStorage `site.lang` → browser language → IP country via `window.__CF_COUNTRY__` → region-aware English fallback) fetches `public/translations/<page>-translations.json` keyed by Hotelrunner locale code (17 languages: en-GB, en-US, pt-PT, pt-BR, nl-NL, fr-FR, it-IT, de-DE, es-ES, sv, pl, af-ZA, zu, sw, ja-JP, zh-CN, ru). Whole-object fallback: every key must exist in every language object. Loader script goes at the end of `<body>`.
...
3. Hostname: `aws-1-eu-west-1.pooler.supabase.com` — exact Lodge session-pooler host, confirmed by the project's historical connection logs and the successful August 23, 2026 Google Ads import
...
- **Marin voice-frame height (widget-loader.js)**: `#dv-voice-frame` has a fixed pixel height set in `voice-reception/artifacts/receptionist/public/widget-loader.js`. Changes to this file (or `WidgetEmbed.tsx`) are **not visible in the Replit preview** — the main-site dev server always loads `widget-loader.js` from the production Render URL (`MIA_URL` in `WebsiteProject/vite.config.js`). To preview voice-frame layout changes, open the receptionist artifact directly at port 21046 (`/embed` path). The fix only goes live after deploying the receptionist to Render.
...
- **LanguageTopBar mobile overflow**: the region selector was removed; the topbar now contains only the `LanguagePicker` component (compact globe-button, ~140px max-width) on the left and the "Book Now" button on the right. The picker button is capped at `max-width: 48vw` so it cannot cause horizontal overflow at any viewport width. If you add new controls to the topbar, the budget at 320px is approximately 288px content width minus the right-side "Book Now" (~80px) minus gaps.
- **Gallery image sizes**: each gallery entry in `src/data/content.js` has three fields — `desktop` (4–28 KB WebP thumbnail, used in the grid), `mobile` (WebP thumbnail), and `full` (43–98 KB JPEG original, used by the lightbox). When adding a new gallery image, all three files must exist in `public/photos/gallery/` and all three fields must be present in the `IMG.gallery` entry. The lightbox `<img>` in `GallerySection.jsx` reads `image.full`; changing it back to `image.desktop` will silently revert to showing the thumbnail.
