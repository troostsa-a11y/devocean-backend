...
- `#hero-placeholder` is a static HTML div (not React) showing a preloaded hero image, dismissed by a CSS animation on the compositor thread. It contains a real `<h1 id="hero-title">` and `<p id="hero-subtitle">` only — the intro deliberately shows just the lodge name + tagline (the placeholder previously mirrored the React hero's CTA buttons, but those non-interactive button-shaped `<div>`s were removed as confusing during startup). Removing them does not affect CLS/LCP: the placeholder is `position: fixed` (out of layout flow) and the buttons sat *below* the title/subtitle, so the `<h1>`/`<p>` y-positions and the hero-image FCP paint are unchanged. Keep the `#hero-title`/`#hero-subtitle` IDs — the localization inline script updates them to the visitor's language.
- **LCP is the static `#hero-title` text, NOT the hero image**: a full-viewport `<img>` (the `#hero-placeholder` background) is excluded from LCP by Chrome's "full-viewport = background" heuristic (web.dev/articles/lcp) — it only ever counts toward FCP. So the LCP element is whichever *text* block is largest. **The real LCP killer was the CookieYes IAB-TCF consent modal**: with "Support IAB TCF v2.3" enabled, CookieYes fetched the ~118 KiB IAB Global Vendor List and rendered a near-full-screen "we and our N partners…" legal paragraph that painted ~10 s on Slow 4G and won LCP. An in-code attempt to beat it (enlarge `#hero-title`, compact the paragraph) was deployed and **verified ineffective** — a single h1 line cannot out-area a full-width multi-line paragraph, and font compaction barely dents its box. **The fix was config-level: IAB TCF v2.3 (+ Google's Additional/Advertiser Consent Mode) was disabled in the CookieYes dashboard (Jun 2026)**, switching it to a standard GDPR banner (short text, no GVL fetch) that no longer wins LCP → LCP collapses to the static `#hero-title` (~1.7–2.5 s). **Do NOT re-enable IAB TCF** unless DEVOCEAN actually serves programmatic display ads (it runs GA4/GTM analytics, which need only Google Consent Mode, not TCF). The enlarged `#hero-title` (`clamp(3.5rem, 14vw, 3.75rem)`; the React `HeroSection` h1 mirrors that **exact** clamp to avoid a shrink-jump at the fade) is now **kept by owner preference for looks**, not as an LCP lever; the `.cky-notice-des` 13px/lh-1.4 compaction is harmless leftover. **No image change (resolution, aspect, asset swap) can ever fix hero LCP — the image is structurally ineligible.**
- **5 s delay is for FCP / intro UX, not LCP**: `hero01-mobile.webp` takes ~1.5 s on Slow 4G; the overlay stays `opacity: 1` so the preloaded image is the FCP paint and the intro reads as instant. (Earlier notes claimed the delay made Chrome pick the image as the LCP candidate — that was wrong; full-viewport images are never LCP candidates.) Animation: `heroDismiss 0.4s cubic-bezier(0.25,1,0.5,1) 5s forwards` (`will-change: opacity` → own GPU layer). JS `setTimeout` is cleanup-only at 5500 ms (5000 + 400 fade + 100). Returning visitors: `animation = 'none'` before `display:none` — they never see it.
- **Padding must match (CLS)**: desktop/tablet (`sm:`+) — the placeholder content div `#hero-placeholder-content` uses `calc(var(--stack-h) + 4rem)` (fixed → measured from viewport top), React `HeroSection` uses `pt-[calc(var(--header-h) + 4rem)]` — the in-flow 40px topbar offsets the section so only `--header-h` is added → same y. On small phones (`<640px`) BOTH drop the extra to `-1rem` (placeholder `@media (max-width:639.98px)` rule; React base `pt-[calc(var(--header-h)_-_1rem)]`) → still matched, title sits 80px higher. The placeholder query uses `639.98px` (not `639px`) to **exactly** complement Tailwind's `sm:` = `min-width:640px` — `639px` would leave a fractional-width band `(639,640)px` where React is mobile but the placeholder reverts to desktop (80px fade jump). **Do NOT give `HeroSection` paddingTop `--stack-h`** (double-counts the topbar → content 40px+ lower → major CLS jump on fade), and **keep the placeholder's `max-width:639.98px` rule in lockstep with the React mobile padding** (change one → change the other by the same amount).
...
- The review block (stars + `badge` + "Click the reviews!" + Trustindex widget) in `HeroSection.jsx` is `absolute bottom-20 sm:bottom-10`, anchored to the **section bottom**; the CTA grid flows from the top. The enlarged hero title (2 lines on mobile) plus a large top gap pushed the CTAs low enough that the bottom-anchored block overlapped the 2nd CTA row.
- **Fix = shrink the top gap on small phones (`<640px`), desktop untouched.** Content div top padding is `pt-[calc(var(--header-h)_-_1rem)] sm:pt-[calc(var(--header-h)_+_4rem)]` — mobile shifts the whole content block **up 80px** (5rem) vs the old `+4rem`, freeing bottom space so the review block clears the CTAs; `sm:`+ keeps the original `+4rem`. To dial it, change the `-1rem` term (more negative = bigger upward shift / tighter top gap). The title still clears the fixed header because the h1 keeps its own `mt-14`.
- **The earlier height-scoped hack was removed** (`[@media(max-width:639px)_and_(max-height:800px)]` → `pb-60`/`bottom-6`): it never triggered on phones **taller than 800px** (modern Androids ~915px), which is exactly where the overlap was reported. Scope the fix by **width only** (small phone = `<640px`), not height.
- **Regime note**: where content < viewport the section is `min-h-screen` (viewport-driven), the review block stays anchored to the viewport bottom, so moving content up directly buys clearance. On very short phones (content > viewport, content-driven) the shift is neutral on clearance but usually pulls content back inside the viewport, restoring the viewport-driven regime.
- **CLS**: this top-gap change is mirrored in the static placeholder — see "Padding must match (CLS)" above. The `#hero-placeholder-content` mobile `padding-top` MUST move in lockstep with the React mobile padding or the title jumps when the placeholder fades.
...
- Net effect: the preloaded WebP paints (for FCP) at ~1.5 s before JS runs — it is the FCP paint, not the LCP element (a full-viewport image is excluded from LCP; the LCP is the static `#hero-title` text).
