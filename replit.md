...
- **Internationalization:**
    ...
    - **Smart Locale Mapping:** Afrikaans uses `af-za` language code throughout the system (URLs, localStorage, translations), mapping to af-ZA locale for booking engine. Afrikaans + MZN currency → pt-MZ locale (preserves Mozambican currency for Afrikaans speakers), Afrikaans + ZAR → af-ZA locale (South Africa).
    ...
    - **Smart Currency Management (Oct 19, 2025):** System tracks original IP-detected currency/region in localStorage (`site.currency.original`, `site.region.original`). When users change regions, currency updates to match the new region. When users return to their original region, their original currency is restored. This preserves country-specific currencies (BRL, MZN, ZAR, GBP) while allowing region changes to update currency appropriately. Intelligent backfill logic automatically corrects region-default currencies (e.g., ZAR for Africa) to user's actual IP-based currency (e.g., MZN for Mozambique), ensuring visitors always see their local currency.
