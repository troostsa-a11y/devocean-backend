...
- `#hero-placeholder` is a static HTML div (not React) showing a preloaded hero image, dismissed by a CSS animation on the compositor thread. It contains a real `<h1 id="hero-title">`, `<p id="hero-subtitle">`, and two button-shaped `<div>`s mirroring the React hero exactly to eliminate CLS on removal. Keep the `#hero-title`/`#hero-subtitle` IDs — the localization inline script updates them to the visitor's language.
...
- **Homepage guard**: the inline script bails before first paint on any path other than `/` (injects `#hero-placeholder{display:none!important}` and returns). "Rates & Availability" / Book Now buttons are plain `<a href="/book-direct">` full-page navigations; without this guard a fresh load of any deep link would flash the homepage hero for 5 s.
...
- **No duplicate email/GA4 work**: creating the Beds24 booking makes Beds24 send its own email, which the IMAP automation already ingests (guest row + scheduled emails + GA4). The direct path schedules no emails.
