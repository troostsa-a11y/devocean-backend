...
- **NR (non-refundable) rate plans ARE shown to guests**: displayed as the cheaper option below SF in the rate picker. Deposit for NR follows the same 50%/100% arrival-date rule as SF — only LM (last-minute) forces 100% upfront. NR is sorted first (cheapest) by `priceOffers()` since offers are sorted ascending by price.
