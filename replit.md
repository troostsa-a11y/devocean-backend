...
- **Guest currency override (display only)**: the `/book-direct` top-bar language·currency pair is a clickable searchable country picker (`WebsiteProject/src/components/CurrencyPicker.jsx`). Picking a country forces its national currency (`CC_TO_CURRENCY`) as the *display* currency. It is display-only — offers still render in `room.currency` (Beds24 base) and Stripe always charges base; the override only retargets the approximate FX line + the label. Persisted via `setCurrency()` in `useLocale.js` with `site.currency.source="user"` so the IP-based initializer never clobbers it (mirrors `site.lang_source`). Shown on all viewports including mobile — the booking top-bar brand text truncates (`min-w-0`/`truncate`) and the picker+menu group is `shrink-0` so the chip never overflows on narrow screens; the dropdown panel is `w-[calc(100vw-2rem)] max-w-[18rem]` right-anchored so it fits any phone width (incl. sub-320px). Picker strings: `currencyLabel`/`currencySearch`/`currencyNoMatches` in the 7 booking base langs (others fall back to EN); country names localize via `Intl.DisplayNames`.

### Date Range Picker (book-direct search)
- The `/book-direct` search form uses a single dual-month range calendar (`WebsiteProject/src/components/DateRangePicker.jsx`) in place of two native `<input type="date">` fields. One month on mobile, two side-by-side at `lg:`+. Built on **luxon** (the only date lib available — no react-day-picker); Monday-first weeks, month/weekday names localized via `DateTime.setLocale(lang)` / `Info.weekdays`.
- Dates stay `YYYY-MM-DD` strings — the picker is UX-only, so the `/api/booking/{availability,checkout}` payloads and the `{checkIn} → {checkOut}` displays are untouched.
- The first day click commits the check-in immediately with a default 1-night checkout (mirrors the old inputs, which bumped checkout to `checkIn+1`), so searching with an incomplete selection still submits a valid ≥1-night range; the second click sets the real checkout and closes the panel. Clicking a date `<= pendIn` restarts the range. This guarantees checkout is always strictly after check-in.
- Popover mirrors `CurrencyPicker`: always-rendered panel toggled via opacity/visibility/pointerEvents/transform (INP), outside-click(mousedown)+Escape close, brand accent `#9e4b13`. Panel is `w-[calc(100vw-3rem)] max-w-[20rem] lg:w-auto`, left-anchored under the trigger, sized so it fits down to ~320px.
- New i18n keys `night` (singular) + `selectDates` added to the 7 base booking langs (en,pt,de,fr,es,it,nl); other website langs fall back to EN via `getBookingStrings` (whole-object fallback).
