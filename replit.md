...
- **Internationalization:** React-based i18n with lazy-loaded translations for 15 languages (en, de, es, fr, it, nl, pt, ru, zh, sv, ja, pl, af, zu, sw), locale persistence, multi-tier detection, IP-based currency, and comprehensive country-derived locale mapping (90+ variants including pt-MZ, sw-TZ, es-MX, zh-HK, etc.). Critical translations are pre-loaded. Experience and form translations use a two-layer and three-tier fallback system respectively. Booking page language detection via URL parameters (`?lang=XX`) with automatic ISO 639-1 conversion. **Language Persistence (Nov 2025 Fix):** User-selected languages are protected from IP-based auto-detection via `site.lang_source="user"` and `site.lang.version="2"` flags. The `LOCALE_BY_LANG` mapping preserves all 15 language codes (including "zu" and "sw") through to booking URLs, ensuring correct `?lang=XX` parameters.
