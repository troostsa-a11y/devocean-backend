...
- The review block (stars + `badge` + "Click the reviews!" + Trustindex widget) in `HeroSection.jsx` is `absolute bottom-20 sm:bottom-10`, anchored to the **section bottom**; the CTA grid flows from the top. On a short viewport the block's top reaches up into the 2nd CTA row → overlap (worsened by the enlarged hero title pushing the buttons down).
- **Clearance formula (short phone = content-driven section)**: `clearance = pb − bottom_offset − block_height`. The CTA grid's own `mt`/`gap` are IRRELEVANT in this regime (shrinking content shrinks the section, which moves the bottom-anchored block up by the same amount). Defaults `pb-52`(208) − `bottom-20`(80) − ~156 ≈ **−28px** → overlap. Only `pb`, `bottom_offset`, and `block_height` move the needle.
- **Fix (scoped to narrow AND short viewports only)** via `[@media(max-width:639px)_and_(max-height:800px)]`: content div adds `pb-60`(240); review block adds `bottom-6`(24) → clearance ≈ **+60px**. Tall phones / desktop are untouched (they already clear). **Do NOT widen this to all mobile** — it would push the review block down on tall phones that are already fine. On very short phones the review widget may sit partly below the fold; unavoidable given the large title (can't fit 56px title + subtitle + 4 CTAs + review card above the fold).
