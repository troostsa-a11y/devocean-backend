...
- **Email Automation System (Oct 25, 2025 - COMPREHENSIVE):** Standalone Node.js service that processes Beds24 booking notifications via IMAP and sends automated emails at 4 touchpoints using Resend API. Architecture: IMAP Parser → Supabase PostgreSQL → Email Scheduler → Resend Transactional Emails. Cron schedule: 08:00, 14:00, 22:00 UTC daily. Database tables: bookings (with extras JSONB field), scheduled_emails, email_logs, email_check_logs. Email touchpoints: (1) Post-booking confirmation within 2 hours, (2) Pre-arrival info 7 days before check-in, (3) Arrival reminder 2 days before, (4) Post-departure thank you 1 day after check-out. Features: ✅ Cancellation handling (auto-stops emails when guests cancel), ✅ Transfer notifications (sends booking requests to taxi company), ✅ Extras management (extra beds, transfers, special requests, dietary requirements), ✅ Standalone HTML email templates (4 templates in `/email_templates/`), ✅ Booking status tracking (active/cancelled/completed). Multi-language ready (EN, PT, extendable). Full implementation at `server.js` with services in `server/services/`. Comprehensive documentation in `EMAIL_AUTOMATION_SETUP.md` and `NEW_FEATURES_SUMMARY.md`. Requires environment variables: DATABASE_URL, RESEND_API_KEY, IMAP credentials, optional: TAXI_EMAIL for transfers. Run with: `node server.js`.
