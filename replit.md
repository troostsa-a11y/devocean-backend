...
- **Email Automation System (Oct 25, 2025 - NEW):** Standalone Node.js service that processes Beds24 booking notifications via IMAP and sends automated emails at 4 touchpoints using Resend API. Architecture: IMAP Parser → Supabase PostgreSQL → Email Scheduler → Resend Transactional Emails. Cron schedule: 08:00, 14:00, 22:00 UTC daily. Database tables: bookings, scheduled_emails, email_logs, email_check_logs. Email touchpoints: (1) Post-booking confirmation within 2 hours, (2) Pre-arrival info 7 days before check-in, (3) Arrival reminder 2 days before, (4) Post-departure thank you 1 day after check-out. Multi-language templates (EN, PT, extendable). Full implementation at `server.js` with services in `server/services/`. Comprehensive documentation in `EMAIL_AUTOMATION_SETUP.md`. Requires environment variables: DATABASE_URL, RESEND_API_KEY, IMAP credentials. Run with: `node server.js`.
