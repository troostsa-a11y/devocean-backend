...
**Fix:** the review block is `static` (normal document flow) **by default**, rendered as the next sibling immediately after the CTA-grid content div, with `mt-6` spacing. It always sits directly below wherever the CTA grid actually ends — for any viewport height, button wrapping, or language string length — so this class of overlap is structurally impossible now, not just tuned around. It only switches to the original `absolute bottom-10 left-0 right-0` desktop placement (positioned relative to the `<section>`, exactly as before) when **both** `min-width:640px` **and** `min-height:601px` hold, via a compound arbitrary media variant — NOT plain `sm:` (width-only). This matters because plain `sm:` would route **landscape phones** (e.g. 667×375, 932×430 — width ≥640px but just as short as a portrait phone) into the desktop absolute path and reintroduce the exact same overlap bug there. The `600px` height cutoff matches the existing landscape-phone threshold already used elsewhere in this file (`[@media_(max-height:600px)_and_(min-width:640px)]` for the h1 hide and CTA margin-top). The section's `flex items-start` was removed since a plain block section produces identical top-aligned stacking for the single full-width content div — no visual change on true desktop/tablet.
