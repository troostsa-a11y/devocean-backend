...
**Must be `position: fixed`, not in-flow:** the placeholder was originally a normal in-flow block sibling of `#root` (`display:flex; min-height:100vh`, no `position`/`z-index`). Since it sits *after* `#root` in the DOM, the instant React mounts — even just the route's `<Suspense>` fallback spinner, well before the lazy `BookDirectPage` chunk finishes loading — `#root` gains real height and pushes the placeholder down out of the viewport, exposing the near-white Suspense fallback underneath. Symptom: a visible "white → hero → white → real page" flash sequence on every /book-direct load. Fix: match the homepage `#hero-placeholder` pattern — `position:fixed; top:0; left:0; width:100%; height:100vh; z-index` above `#root`'s (`z-index:1`) — so it stays on top of whatever React renders underneath until `BookDirectPage`'s `useLayoutEffect` removes it. Any future single-purpose route placeholder needs this same fixed-overlay treatment, not just a display:none/flex toggle.
