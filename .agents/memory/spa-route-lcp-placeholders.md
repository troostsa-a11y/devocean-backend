...
**CSS specificity for override rules:** when writing a more-specific override (`dl-card-impact p`) against a general rule (`dl-card p`) with the same selector structure, count the classes — if they tie (both 0,2,1), declaration order decides and the general rule always comes last. Add the shared parent class to the override selector (`dl-card.dl-card-impact p` → 0,3,1) so it wins regardless of order. Never rely on declaration position for intent-critical overrides.
