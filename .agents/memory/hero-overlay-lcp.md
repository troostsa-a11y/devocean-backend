...
**The real lever is CookieYes config, not code.** The ~10 s render delay is CookieYes fetching the ~118 KiB IAB GVL JSON then painting a huge "we and our N partners…" paragraph. Best fix: in the CookieYes dashboard switch from IAB TCF / Google-certified CMP mode to a standard GDPR cookie banner (DEVOCEAN runs GA4/GTM, not programmatic ad exchanges, so TCF is overkill) — this removes the GVL fetch + giant paragraph → small fast banner → LCP collapses to the hero (~1.7–2.5 s). Compliance-safe (still a real consent banner). Deterministic code-only fallback = defer the banner until first interaction (banner never paints during a no-interaction Lighthouse run) — but TCF wants prominent immediate display, so that's the owner's compliance call.
