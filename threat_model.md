...
- **Public vs admin operations** — public website forms are intentionally open; booking-management and manual automation actions must stay behind explicit server-side authorization.
...
- **Public surfaces:** website forms and any unauthenticated Express routes such as `/health`, `/`, and `/api/check-emails`.
- **Admin boundary:** `render-automailer/server.ts` routes protected by `requireAdminKey`.
...
The system stores guest PII and operational data. Responses, logs, and admin routes must avoid leaking guest details, secrets, or internal service behavior to the public. Public endpoints should return only the minimum needed result, and admin operations must remain server-side protected.
